...
impl<'r> Decode<'r, Odbc> for OffsetDateTime {
    fn decode(value: OdbcValueRef<'r>) -> Result<Self, BoxDynError> {
        ...
        if let Ok(datetime) = <PrimitiveDateTime as Decode<Odbc>>::decode(value) {
...
mod tests {
    ...
    fn time_values_decode_old_text_and_numeric_forms() -> Result<(), BoxDynError> {
        let datetime_value = OdbcValue::new(OdbcValueKind::Text("2023-12-25 14:30:00".to_owned()));
        assert_eq!(
            <PrimitiveDateTime as Decode<Odbc>>::decode(datetime_value.as_ref())?,
            datetime!(2023-12-25 14:30:00)
        );

        let date_value = OdbcValue::new(OdbcValueKind::Text("20231225".to_owned()));
        assert_eq!(
            <Date as Decode<Odbc>>::decode(date_value.as_ref())?,
            date!(2023 - 12 - 25)
        );

        let time_value = OdbcValue::new(OdbcValueKind::BigInt(52_200));
        assert_eq!(
            <Time as Decode<Odbc>>::decode(time_value.as_ref())?,
            time_macro!(14:30:00)
        );
    ...
    fn time_values_decode_raw_odbc_temporal_kinds() -> Result<(), BoxDynError> {
        let date_value = OdbcValue::new(OdbcValueKind::Date(odbc_api::sys::Date {
            year: 2023,
            month: 12,
            day: 25,
        }));
        ...
        let time_value = OdbcValue::new(OdbcValueKind::Time(odbc_api::sys::Time {
            hour: 14,
            minute: 30,
            second: 0,
        }));
        ...
        let timestamp_value = OdbcValue::new(OdbcValueKind::Timestamp(odbc_api::sys::Timestamp {
            year: 2023,
            month: 12,
            day: 25,
            hour: 14,
        ...
        assert_eq!(
            <PrimitiveDateTime as Decode<Odbc>>::decode(timestamp_value.as_ref())?,
    ...
    fn time_values_encode_to_old_odbc_argument_forms() -> Result<(), BoxDynError> {
        ...
        assert_eq!(buf, vec![OdbcArgumentValue::Text("2023-12-25".to_owned())]);
        ...
        assert!(matches!(result, IsNull::No));
        assert!(
            matches!(&buf[..], [OdbcArgumentValue::Text(text)] if text.starts_with("14:30:00"))
        );
    ...
    fn time_decode_error_matches_old_odbc() {
        let value = OdbcValue::new(OdbcValueKind::Text("not_a_datetime".to_owned()));
        let result = <PrimitiveDateTime as Decode<Odbc>>::decode(value.as_ref());
